					...
					xmlHttp2.onreadystatechange = function()
					{
						if (xmlHttp2.readyState==4 && xmlHttp2.status==200)
						{
							...
							for(i=0; i<Response.length; i++)
							{
								...
								window.searchResults[window.searchResults.length] = new Connection(CnxResponse.bamsID, CnxResponse.sourceID, CnxResponse.sourceName, CnxResponse.sourceAbbrev, CnxResponse.targetID, CnxResponse.targetName, CnxResponse.targetAbbrev, CnxResponse.nomenclature, CnxResponse.species, null, CnxResponse.dimensions, CnxResponse.coordinatePlot, CnxResponse.notes, [new ConnectionEvidence(CnxResponse.evidence.sourceID, CnxResponse.evidence.sourceName, CnxResponse.evidence.sourceAbbrev, CnxResponse.evidence.targetID, CnxResponse.evidence.targetName, CnxResponse.evidence.targetAbbrev, CnxResponse.evidence.nomenclature, CnxResponse.evidence.strength, CnxResponse.evidence.injectionLocation, CnxResponse.evidence.terminalFieldLocation, CnxResponse.evidence.technique, CnxResponse.evidence.annotation, CnxResponse.evidence.curator, CnxResponse.evidence.referenceName, CnxResponse.evidence.referenceURL)], "searchResults", false);
							}
						}
					};
					var cnxUrl = "api/v1/nomenclature/1/region/" + url2[0] + "/connection/";
					xmlHttp2.open("GET",cnxUrl,true);
					xmlHttp2.send();
				}
...
function searchFromMap(mapValue)
{
	...
	console.log("!! searched for it, found " + found);
	if(found == 0)
	{
		//make API call to find region
		xmlHttp = new XMLHttpRequest();

...
echo "Running: Parse Connections";
...
// for($i=0; $i<20; $i++)
{
	...
	$molecules = $parts[$i]->molecules_in_region->molecule_in_region;
	if(count($molecules) > 0)
	{
		echo "<b>Has molecules:</b><br>";
		
		for($j=0; $j<count($molecules); $j++)
		{
			$thisMolecule = $molecules[$j];

			
			$partID = substr($parts[$i]['id'], 1);
			$molID = substr($molecules[$j]['id'], 1);
			...
			$request = "https://www.kimonolabs.com/api/7yzpbz4o?apikey=K5qC0NujHIC6RnADjU6APzIMl0BiSGJJ&aidi=" . $thisPart['bamsID'] . "&chem=" . $molID;
